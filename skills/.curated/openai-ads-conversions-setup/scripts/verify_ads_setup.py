...
def legacy_capi_field_hits(root: Path, limit: int = 20) -> list[dict[str, object]]:
    ...
    for path in iter_candidate_files(root):
        text = read_file(path)
        if not CAPI_CONTEXT_RE.search(text):
            continue
...
def main() -> int:
    ...
    checks = [
        ...
        make_check(
            ...
            legacy_capi_fields,
            "No removed legacy CAPI request field names found in CAPI-like files.",
        ),
        make_check(
